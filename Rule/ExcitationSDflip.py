...
class ExcitationSDflipRule(MetropolisRule):
    sdexes_S: jax.Array
    sdexes_D: jax.Array
    # nonoAnoB: jax.Array
    noarr: jax.Array
    ...
    def transition(rule, sampler, machine, parameters, state, key, σ):
        ...
        seq_arr = jax.random.choice(key1, length_sd, shape=(n_chains,))
        TF_arr = jax.random.choice(key2, 2, shape=(n_chains,))
        ...
        def flip_trans(σ, seq):
            return σ*(-1.)

        def decorator_case2(σ, seq, TF):
            return jax.lax.cond(TF == 1, decorator_case, flip_trans, σ, seq)

        return (
            jax.vmap(decorator_case2, in_axes=(0, 0, 0), out_axes=0)(σ, seq_arr, TF_arr),
            None,
        )

    def __repr__(self):
        return f"ExcitationSDflipRule()"